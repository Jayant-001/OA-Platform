...
export class UserRepository {
    async findAllUsers(): Promise<UserDetails[]> {
        return db.any("SELECT * FROM user_details JOIN users On user_id=users.id where role='user'");
    }
    ...
    async userExists(userId: string): Promise<boolean> {
        const result = await db.oneOrNone("SELECT 1 FROM users WHERE id = $1", [userId]);
        return result !== null;
    }
}
